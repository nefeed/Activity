...
$(function(){
	$.getJSON("./",function(){
		...
		for(i=0;i<json.length;i++){
			catemp+="<input type='radio' name='assistant' id='"+json[i].m_secId+"' value='"+json[i].m_secName+"' onclick='find("+json[i].m_secId+")'/>"
		}
		$("#xiasu").append(catemp);
	});
	$('#fg').dialog({
...
function find(radio){
	if(document.getElementById("radio").checked){
		$.getJSON("./",function(){
			...
			k=json.length;
			for(i=0;i<json.length;i++){
				catemp+="<input type='radio'  id='"+json[i].m_cusId+"' value='"+json[i].m_secName+"'/>"
			}
			$("#custom").append(catemp);
		});
	}
	$('#fg').dialog('open');
}
function submit(){
	...
	for(i=0;i<k-1;i++){
		if(document.getElementById("json[i].m_cusId").checked) {
			m_receiveAble+=json[i].m_secId+","
		}
	}
	if(document.getElementById("json[k-1].m_cusId").checked) {
		m_receiveAble+=json[i].m_secId
	}
